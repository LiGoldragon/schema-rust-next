...
impl RustEmitter {
    ...
    pub fn emit(&self, asschema: &Asschema) -> RustCode {
        ...
        // The signal / mail / plane runtime floor is emitted ONLY for a
        // component module (one that declares a signal plane). A pure
        // types-only module (the D3 shape — `signal_plane() == None`) is
        // Imports + Namespace + NOTA codec only: no root enums, no signal
        // frames, no mail floor, no Plane surface. Its types are imported
        // by component modules across the crate boundary (the D2 shared
        // floor), so the floor lives once in the component, never
        // duplicated into the type library.
        if asschema.signal_plane().is_some() {
            for root_enum in asschema.input_and_output() {
            ...
            writer.emit_plane_surface(&asschema.input_and_output());
...
/// vector/map/option codecs) is emitted only when the schema actually
/// uses `Vec` / `KeyValueMap` / `Option` at some reference position.
/// Schemas with no collections emit byte-identical output to the
/// pre-collection emitter, preserving the checked-in legacy fixtures.
...
impl RustWriter {
    ...
    /// `BTreeMap` is a brace block of `key value` pairs `{k1 v1 ...}`
    /// (even child count); an `Option` is the atom `None` or the paren
    /// `(Some inner)`. The parse methods take a per-element closure so
    /// nested collections compose; the format associated functions
    /// mirror them. Only emitted when the schema uses a collection, so
    /// collection-free schemas keep byte-identical output.
        ...
        for variant in variants {
            match &variant.payload {
                Some(payload) => self.line(format!(
                    ...
                    variant.name,
                    variant.name,
                    self.format_reference_expression(payload, "payload")
                )),
    ...
    fn emit_mail_event_support(&mut self, root_enums: &[&EnumDeclaration]) {
        ...
        self.blank();
        // OriginRoute is the auto-created field on every root plane
        // message (records 1038/1039). It is minted at ingress, carried
        // on the Plane root through every engine, and echoed back on the
        // reply Plane. It is generic runtime substrate, not a schema-
        // authored type.
        self.line("#[derive(rkyv::Archive, rkyv::Serialize, rkyv::Deserialize, Clone, Copy, Debug, PartialEq, Eq)]");
        self.line("pub struct OriginRoute(pub Integer);");
        self.blank();
        self.line("impl OriginRoute {");
        self.line("    pub fn at_ingress(value: Integer) -> Self {");
        self.line("        Self(value)");
        self.line("    }");
        self.line("}");
        self.blank();
    ...
    /// The record-1054 plane surface: a single data-carrying enum named
    /// `Plane` whose `Signal` / `Nexus` / `Sema` variants carry the
    /// actual plane messages, so runtime code matches directly on the
    /// plane instead of pairing a thin kind tag with a separate envelope
    /// (record 1052 names that shape wrong). The auto-created origin
    /// route (records 1038/1039) is folded onto the root as the leading
    /// tuple element of each variant — minted at ingress, threaded
    /// through, echoed back. The three-engine chain (records 1028/1030)
    /// is the `SignalEngine` → `NexusEngine` → `SemaEngine` trait order;
    /// `Plane::drive` is the running chain that matches on the plane and
    /// threads a request from Signal through Nexus and Sema to a reply.
    fn emit_plane_surface(&mut self, root_enums: &[&EnumDeclaration]) {
        let has_input = self.has_root_enum(root_enums, "Input");
        let has_output = self.has_root_enum(root_enums, "Output");
        if !(has_input && has_output) {
            return;
        }
        self.line("#[derive(rkyv::Archive, rkyv::Serialize, rkyv::Deserialize, Clone, Debug, PartialEq, Eq)]");
        self.line("pub enum Plane {");
        self.line("    Signal(OriginRoute, Input),");
        self.line("    Nexus(OriginRoute, Input),");
        self.line("    Sema(OriginRoute, Output),");
        self.line("}");
        self.blank();
        self.line("impl Plane {");
        self.line("    pub fn at_ingress(origin_route: OriginRoute, request: Input) -> Self {");
        self.line("        Self::Signal(origin_route, request)");
        self.line("    }");
        self.blank();
        self.line("    pub fn origin_route(&self) -> OriginRoute {");
        self.line("        match self {");
        self.line("            Self::Signal(origin_route, _) => *origin_route,");
        self.line("            Self::Nexus(origin_route, _) => *origin_route,");
        self.line("            Self::Sema(origin_route, _) => *origin_route,");
        self.line("        }");
        self.line("    }");
        self.line("}");
        self.blank();
        self.emit_three_engine_chain();
    }

    fn emit_three_engine_chain(&mut self) {
        self.line("pub trait SignalEngine {");
        self.line("    type Error;");
        self.blank();
        self.line("    /// Validate the ingress request and admit it onto the Nexus");
        self.line("    /// plane, carrying the same origin route forward.");
        self.line("    fn admit(&self, signal: Plane) -> Result<Plane, Self::Error>;");
        self.line("}");
        self.blank();
        self.line("pub trait NexusEngine {");
        self.line("    type Error;");
        self.blank();
        self.line("    /// Execute the admitted request and hand the projected");
        self.line("    /// reply onto the Sema plane.");
        self.line("    fn execute(&self, nexus: Plane) -> Result<Plane, Self::Error>;");
        self.line("}");
        self.blank();
        self.line("pub trait SemaEngine {");
        self.line("    type Error;");
        self.blank();
        self.line("    /// Apply the reply to durable state and return the reply");
        self.line("    /// Plane to be echoed back to the caller.");
        self.line("    fn apply(&mut self, sema: Plane) -> Result<Plane, Self::Error>;");
        self.line("}");
        self.blank();
        self.line("#[derive(Debug, PartialEq, Eq)]");
        self.line("pub enum PlaneChainError<SignalError, NexusError, SemaError> {");
        self.line("    Signal(SignalError),");
        self.line("    Nexus(NexusError),");
        self.line("    Sema(SemaError),");
        self.line("    UnexpectedPlane,");
        self.line("}");
        self.blank();
        self.line("impl Plane {");
        self.line("    /// Drive the request through the three trait-ordered engines:");
        self.line("    /// Signal validates and pushes to Nexus, Nexus executes and");
        self.line("    /// pushes to Sema, Sema applies and returns the reply Plane.");
        self.line("    /// The origin route minted at ingress threads every hop and is");
        self.line("    /// echoed on the returned reply Plane.");
        self.line("    #[allow(clippy::type_complexity)]");
        self.line("    pub fn drive<Signal, Nexus, Sema>(");
        self.line("        self,");
        self.line("        signal: &Signal,");
        self.line("        nexus: &Nexus,");
        self.line("        sema: &mut Sema,");
        self.line(
            "    ) -> Result<Plane, PlaneChainError<Signal::Error, Nexus::Error, Sema::Error>>",
        );
        self.line("    where");
        self.line("        Signal: SignalEngine,");
        self.line("        Nexus: NexusEngine,");
        self.line("        Sema: SemaEngine,");
        self.line("    {");
        self.line("        let ingress_route = self.origin_route();");
        self.line("        let admitted = signal.admit(self).map_err(PlaneChainError::Signal)?;");
        self.line("        let Self::Nexus(_, _) = &admitted else {");
        self.line("            return Err(PlaneChainError::UnexpectedPlane);");
        self.line("        };");
        self.line(
            "        let executed = nexus.execute(admitted).map_err(PlaneChainError::Nexus)?;",
        );
        self.line("        let Self::Sema(_, _) = &executed else {");
        self.line("            return Err(PlaneChainError::UnexpectedPlane);");
        self.line("        };");
        self.line("        let reply = sema.apply(executed).map_err(PlaneChainError::Sema)?;");
        self.line("        debug_assert_eq!(reply.origin_route(), ingress_route);");
        self.line("        Ok(reply)");
        self.line("    }");
        self.line("}");
    }
    ...
    fn emit_nexus_dispatch_impl(&mut self, root_enum: &EnumDeclaration) {
        let trait_name = format!("{}Nexus", root_enum.name);
        self.line(format!("impl {} {{", root_enum.name));
        self.line("    pub fn dispatch_mail_with_nexus<Nexus>(self, identifier: MessageIdentifier, origin_route: OriginRoute, nexus: &Nexus) -> Result<MessageProcessed<Nexus::Reply>, Nexus::Error>");
        self.line("    where");
    ...
    fn has_root_enum(&self, root_enums: &[&EnumDeclaration], name: &str) -> bool {
        root_enums
            .iter()
    ...
    /// Rust value.
    ///
    /// Plain leaves call the scalar/`from_nota_block` codecs. Collection
    /// references decode structurally: a `Vec` reads a square-bracket
    /// block's children, a `KeyValueMap` reads a brace block's key/value
    /// pairs into a `BTreeMap`, an `Option` reads the paren forms
    /// `None` / `(Some inner)`. Each recurses through `parse_expression`
    /// on the inner reference, so nested collections compose. The
    /// generated closures borrow `NotaBlock`/`NotaDecodeError` from the
    /// emitted NOTA support block.
    /// The expression that decodes a NOTA `block` into this reference's
    ...
    fn parse_expression_result(&self, reference: &TypeReference, block: &str) -> String {
        match reference {
            ...
            TypeReference::Vector(inner) => format!(
                ...
                self.parse_element_function(inner, "element")
            ),
            TypeReference::Map(key, value) => format!(
                "NotaCollection::new({block}).parse_map({}, {})",
                self.parse_element_function(key, "key_block"),
                self.parse_element_function(value, "value_block")
            ),
            TypeReference::Optional(inner) => format!(
                "NotaCollection::new({block}).parse_option({})",
                self.parse_element_function(inner, "inner_block")
            ),
        }
    }

    /// The decode function passed to a `NotaCollection` parse method.
    ///
    /// A plain non-scalar type passes its `from_nota_block` as a bare
    /// path (`NodeName::from_nota_block`) — clippy-clean, no redundant
    /// closure. Scalars and nested collections need an inline closure
    /// over the element binding.
    fn parse_element_function(&self, reference: &TypeReference, binding: &str) -> String {
        match reference {
            ...
            _ => format!(
                "|{binding}| {}",
                self.parse_expression_result(reference, binding)
            ),
        }
    }

    /// Encode an OWNED place (a struct field `self.x`, a newtype's
    /// `self.0`) into NOTA. Collections take `&place` because the
    /// `NotaCollection` format functions borrow the collection.
    fn format_expression(&self, reference: &TypeReference, value: &str) -> String {
        self.format_expression_with_borrow(reference, value, "&")
    }

    /// Encode an ALREADY-BORROWED binding (an enum-variant payload
    /// bound by `match &self`) into NOTA. Collections pass the binding
    /// directly — adding `&` would create a needless double reference.
    fn format_reference_expression(&self, reference: &TypeReference, value: &str) -> String {
        self.format_expression_with_borrow(reference, value, "")
    }

    /// The shared encode body. `collection_borrow` is `"&"` for owned
    /// places and `""` for already-borrowed bindings, applied only to
    /// the collection arms (scalars manage their own borrowing).
    fn format_expression_with_borrow(
        &self,
        reference: &TypeReference,
        value: &str,
        collection_borrow: &str,
    ) -> String {
        match reference {
            TypeReference::Plain(name) => match name.local_part() {
            ...
            TypeReference::Vector(inner) => format!(
                "NotaCollection::format_vector({collection_borrow}{value}, {})",
                self.format_element_function(inner, "element")
            ),
            TypeReference::Map(key, value_reference) => format!(
                "NotaCollection::format_map({collection_borrow}{value}, {}, {})",
                self.format_element_function(key, "key"),
                self.format_element_function(value_reference, "value")
            ),
            TypeReference::Optional(inner) => format!(
                "NotaCollection::format_option({collection_borrow}{value}, {})",
                self.format_element_function(inner, "inner")
            ),
        }
    }

    /// The encode function passed to a `NotaCollection` format method.
    ///
    /// A plain non-scalar type passes its `to_nota` as a bare path
    /// (`NodeName::to_nota`) — clippy-clean. Scalars and nested
    /// collections need an inline closure over the element binding.
    fn format_element_function(&self, reference: &TypeReference, binding: &str) -> String {
        match reference {
            TypeReference::Plain(name) if !matches!(name.local_part(), "Text" | "Integer") => {
                format!("{}::to_nota", name.as_str())
            }
            // The element binding is a reference yielded by the
            // collection iterator, so a nested collection passes it
            // through directly (no extra borrow).
            _ => format!(
                "|{binding}| {}",
                self.format_reference_expression(reference, binding)
            ),
        }
    }
    ...
    /// emitted aliases; any other name is the local type, which after
    /// a cross-crate import alias is the imported type's local name).
    /// The collection variants recurse: `Vector` → `Vec<inner>`,
    /// `Map` → `BTreeMap<key, value>`, `Optional` → `Option<inner>`.
    /// `BTreeMap` is written fully-qualified so no `use` is emitted and
    /// the ordering is deterministic (rkyv + NOTA round-trips need a
    /// stable key order).
